...
if(coin.change>=0){cryptoCoin+=`bg-success text-right">${coin.change}%</p>
                            </div>
                        </a>`;}
else{cryptoCoin+=`bg-danger text-right">${coin.change}%</p>
                            </div>
                        </a>`;}
cryptoCoin+=`<div class="modal fade" id="${coin.symbol}Modal" tabindex="-1" role="dialog" aria-labelledby="${coin.symbol}Modal" aria-hidden="true">
                            ...
                            </div>`;document.getElementById("data").innerHTML=cryptoCoin;createChart(coin.symbol+"Chart",coin.sparkline);});if(coinsData.length>0){document.getElementById("data").innerHTML=cryptoCoin;$('.modal').on('shown.bs.modal',function(e){let coinSymbol=e.relatedTarget.dataset.coinSymbol;let coinData=JSON.parse(e.relatedTarget.dataset.coinData);let labelData=Array.from({length:coinData.length});labelData.fill("");createChart(coinSymbol+"Chart",coinData,labelData)})}
else{document.getElementById("data").innerHTML="";}});}}).catch((error)=>{console.log(error);});}
function createChart(coinname,coindata,labels){var ctx=document.getElementById(coinname);var myChart=new Chart(ctx,{type:"line",data:{labels:labels,datasets:[{label:"# of Votes",data:coindata}]},options:{scales:{y:{ticks:{beginAtZero:true}}}}});}
search();});